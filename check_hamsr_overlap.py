...
gdas_dir = lambda date: f"../CPEX-CV/GDAS_DAWNR0_dropsondeR0_HALOR1pqc_HAMSRR1_radiosondeR0_model_level/{date:%Y%m%d}/"
prepbufr_filename = lambda date: f"hamsr.t{date:%H}z.prepbufr.nr"
text_data_file = "decoded_hamsr_data"
...
for date in pd.date_range(start_date, end_date, freq=freq):
    ...
    if Path(f"{gdas_dir(date)}/{prepbufr_filename(date)}").is_file():
        ...
        map_img.draw_scatter(
            ...
            zorder=9)
        map_img.draw_title(f"HAMSR {date:%Y-%m-%d %HZ}")
        map_img.draw_shapefiles()
        map_img.draw_gridlines()
        map_img.save_image(images_dir, f"hamsr_{date:%Y%m%d%H}")
