program prepbufr_write_dropsonde
!
! write all dropsonde observations out from prepbufr.
...
!
    ...
    msg_report: do while (ireadmg(unit_in, subset, idate) == 0)
        sb_report: do while (ireadsb(unit_in) == 0)
            ...
            if ((hdr(5) .eq. 132) .or. (hdr(5) .eq. 182) .or. (hdr(5) .eq. 232) &
                .or. (hdr(5) .eq. 282)) then
                write (11, '(a7)') 'message'
                write (11, '(4f14.1)') (hdr(i), i=2, 5)
                do k = 1, iret
