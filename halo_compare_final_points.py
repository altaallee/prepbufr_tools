...
slat = 0
nlat = 0
prepbufr_file = lambda date: f"prepbufr_files/{date:%Y%m%d%H}/halo.t{date:%H}z.prepbufr.nr"
out_file = lambda date: f"location_files/locations_halo_{date:%Y%m%d%H}.txt"
...
for date in pd.date_range(start_date, end_date, freq=frequency):
    ...
    ds_halo = extra.get_halo_data(
        halo_filename, start=start_window - timedelta(days=1), end=end_window - timedelta(days=1), preliminary=True)
    ...
    ax.plot(
        ds_halo["lon"], ds_halo["lat"], linestyle="none", marker="o",
    ...
    ax.set_title(f"HALO {date:%Y-%m-%d %HZ}")
    gl = ax.gridlines(draw_labels=["bottom", "left"])
    gl.top_labels = False
    gl.right_labels = False

    plt.savefig(f"location_plots/halo_{date:%Y%m%d%H}{f'_{wlon}_{elon}_{slat}_{nlat}' if extent else ''}.png")
